...
% VALIDATIONTUNE Grid search over lambda, evaluated by trajectory RMSE.
    ...
    % ---- Parse options ----
    defs.LambdaGrid = logspace(-3, 15, 50);
    defs.Method = 'validation';
    defs.Precondition = [];
    defs.Algorithm = [];
    opts = sidParseOptions(defs, varargin);
    lambdaGrid = opts.LambdaGrid;
    extraArgs = {};
    if ~isempty(opts.Precondition)
        extraArgs = [extraArgs, {'Precondition', opts.Precondition}];
    end
    if ~isempty(opts.Algorithm)
        extraArgs = [extraArgs, {'Algorithm', opts.Algorithm}];
    end
...
%   grid points are consistent at 95% confidence (chi^2(2) threshold).
    ...
    % ---- Parse options ----
    defs.LambdaGrid = logspace(0, 10, 25);
    defs.Method = 'frequency';
    defs.SegmentLength = [];
    defs.ConsistencyThreshold = 0.90;
    defs.CoherenceThreshold = 0.3;
    defs.Precondition = [];
    defs.Algorithm = [];
    opts = sidParseOptions(defs, varargin);
    lambdaGrid = opts.LambdaGrid;
    segLen = opts.SegmentLength;
    consistThresh = opts.ConsistencyThreshold;
    cohThresh = opts.CoherenceThreshold;
    extraArgs = {};
    if ~isempty(opts.Precondition)
        extraArgs = [extraArgs, {'Precondition', opts.Precondition}];
    end
