...
%% Test 26: Welch NFFT < SubSegmentLength must raise (silent truncation guard)
% Regression for a latent bug where fft(x, nfft) silently truncated each
% sub-segment to the first nfft samples, producing a wrong Welch
% periodogram with no diagnostic. See SPEC.md §6.2 table.
rng(26);
y26 = randn(1000, 1);
u26 = randn(1000, 1);

nfftBadList = [128, 256, 511];
for k = 1:length(nfftBadList)
    try
        sidFreqMap(y26, u26, ...
            'Algorithm', 'welch', ...
            'SegmentLength', 1000, ...
            'SubSegmentLength', 512, ...
            'NFFT', nfftBadList(k));
        error('Should have thrown sid:invalidNFFT for NFFT=%d', nfftBadList(k));
    catch e
        assert(strcmp(e.identifier, 'sid:invalidNFFT'), ...
            'NFFT=%d: expected sid:invalidNFFT, got %s', ...
            nfftBadList(k), e.identifier);
    end
end

% Boundary: NFFT == SubSegmentLength must be accepted.
res_eq = sidFreqMap(y26, u26, ...
    'Algorithm', 'welch', ...
    'SegmentLength', 1000, ...
    'SubSegmentLength', 512, ...
    'NFFT', 512);
assert(length(res_eq.Frequency) == 256, ...
    'NFFT == Lsub should produce nfft/2 = 256 one-sided bins, got %d', ...
    length(res_eq.Frequency));
